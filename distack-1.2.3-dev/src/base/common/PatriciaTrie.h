...
class PatriciaTrie {
	...
	public:
		...
		typedef list<PatriciaTrieNode<S,T>*>			NODE_LIST;
...
PatriciaTrie<S,T>::~PatriciaTrie() {
	...
	NODE_LIST			list	= GetNodeList (head);
	NODE_LIST_ITERATOR	i		= list.begin  ();
	NODE_LIST_ITERATOR	iend	= list.end    ();
...
typename PatriciaTrie<S,T>::NODE_LIST PatriciaTrie<S,T>::GetNodeList (PatriciaTrieNode<S,T>* root) {
	...
	if (startNode == NULL) return retNodes;
