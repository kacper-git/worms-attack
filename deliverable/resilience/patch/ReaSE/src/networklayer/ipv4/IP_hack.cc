...
IPDatagram *IP_hack::encapsulate(cPacket *transportPacket,
		InterfaceEntry *&destIE) {
	IPControlInfo *controlInfo = check_and_cast<IPControlInfo*> (
			transportPacket->removeControlInfo());

	IPDatagram_hacked *datagram = new IPDatagram_hacked(
			transportPacket->getName());
	...
	if (dynamic_cast<IPControlInfo_hacked*> (controlInfo))
		datagram->setAttackTag(
				((IPControlInfo_hacked *) controlInfo)->getAttackTag());
...
void IP_hack::handlePacketFromNetwork(IPDatagram *datagram) {
	...
	if (!datagram->getFragmentOffset() && copyFlag && (strstr(
			datagram->getName(), "DistackRe") == NULL)) {
	...
	if (datagram->hasBitError()) {
		...
		double relativeHeaderLength = datagram->getHeaderLength()
				/ (double) datagram->getByteLength();
